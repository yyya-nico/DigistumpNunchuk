...
class DigiMouseDevice {
 public:
	...
	// void scrollResV(char res)	{
	// 	last_built_report[3] = *(reinterpret_cast<unsigned char *>(&res));	 
	// }
	
	// void scroll(char deltaS)	{
	// 	if (deltaS == -128) deltaS = -127;
	// 	const char cache = deltaS;
	// 	deltaS = cache << 2;
	// 	last_built_report[3] = *(reinterpret_cast<unsigned char *>(&deltaS));
	// 	deltaS = cache >> 6;
	// 	last_built_report[4] = *(reinterpret_cast<unsigned char *>(&deltaS)); 
	// }
	...
	void scrollV(char deltaS)	{
		scroll(deltaS); 
	}
	
	// void scrollResH(char res)	{
	// 	res <<= 2;
	// 	last_built_report[4] = *(reinterpret_cast<unsigned char *>(&res));	 
	// }
	
	void scrollH(char deltaS)	{
		if (deltaS == -128) deltaS = -127;
		last_built_report[4] = *(reinterpret_cast<unsigned char *>(&deltaS));	 
	}
